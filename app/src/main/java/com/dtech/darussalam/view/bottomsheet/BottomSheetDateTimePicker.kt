...
class BottomSheetDateTimePicker constructor(private var selectedDate: String, private var startTime : String) : BottomSheetDialogFragment() {
    ...
    override fun setupDialog(dialog: Dialog, style: Int) {
        ...
        (binding.root.parent as View).setBackgroundColor(ContextCompat.getColor(ActivityBase.activity, android.R.color.transparent))
            ...
            when {
                ...
                binding.rlStartTime.visibility == View.VISIBLE -> {
                    ...
                    if (DateUtil.getMinutesDifference( DateUtil.getCurrentDate()!! , selectedStartTimeMeeting ) > 1){
                        ...
                        ActivityBase.activity.showToastMessage(getString(R.string.cant_select_previous_time))
                    }

                }
                else -> {
                    mListener.onSelectDateAndTime(selectedDateMeeting*/
/*+"T:00:00:00"*//*
 , selectedStartTimeMeeting)
                    dialog.dismiss()
                }
            }
