...
import com.dtech.darussalam.data.model.responsemodel.ApiResponse
import com.dtech.darussalam.data.model.responsemodel.ApplyCouponResponseModel
...
import com.dtech.darussalam.utilities.extensions.hideAppLoader
import com.dtech.darussalam.utilities.extensions.onTokenExpiredLogout
import com.dtech.darussalam.utilities.extensions.showAppLoader
import com.dtech.darussalam.utilities.extensions.showUpdateDialoge
...
object SingleEnqueueCall {
    ...
    fun <T> callRetrofit(call: Call<ApiResponse<T>>, apiName: String, isLoaderShown: Boolean, apiListener: IGenericCallBack) {
        if (isLoaderShown)
                ...
                if (response.isSuccessful) {
                    if (response.body() != null) {
                        if (response.body()!!.data != null) {
                            ...
                            ActivityBase.activity.showUpdateDialoge(response.body()!!.message!!)
                        }else {
                    ...
                    when {
                      ...
                      /*  response.code() == Constants.TOKEN_EXPIRE -> {
                            val callRefresh = DarusslamApp.apiService.refreshToken()
                            val callBack = this
                            callRetrofitRefresh(callRefresh, Constants.REFRESH_TOKEN, true, object : IGenericCallBack {
                                    override fun success(apiName: String, response: Any?) {
                                        var reponse = response as LoginUserResponseModel
                                        DarusslamApp.db.putString(Constants.CONST_USER_TYPE, reponse!!.userType!!)
                                        DarusslamApp.db.putString(Constants.CONST_USER_DATA, Gson().toJson(reponse!!))
                                        DarusslamApp.db.putBoolean(Constants.CONST_IS_LOGIN, true)
                                        DarusslamApp.db.putString(Constants.CONST_USER_PHONE, LoginToYourAccountFragment.phoneNumber)
                                        if (reponse.token != null) {
                                            DoneApp.db.putString(Constants.ACCESS_TOKEN, reponse.token!!.accessToken!!)
                                            DoneApp.db.putString(Constants.TOKEN_TYPE, reponse.token!!.tokenType!!)
                                        }
            ...
            override fun onFailure(call: Call<ApiResponse<T>>, t: Throwable) {
                ...
                if (t.message != "Canceled") {
                    if (t is UnknownHostException || t is IOException) {
                        Toast.makeText(ActivityBase.activity, Constants.CONST_NO_INTERNET_CONNECTION, Toast.LENGTH_SHORT).show()
    ...
    fun <T> enqueueWithRetry(call: Call<ApiResponse<T>>, callback: Callback<ApiResponse<T>>, isLoaderShown: Boolean) {
        ActivityBase.activity.showAppLoader()
