package com.dtech.darussalam.utilities.utils
...
object DateUtil {
    ...
    fun getDateInUtc(OurDate: String): String {
        var OurDate = OurDate
        try {
            val formatter = SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss" , Locale("en"))
    ...
    fun convertSingleTimePicker(startDate: String?): String {
        return if (startDate != null) {
            ...
            val df = SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss" , Locale("en"))
    ...
    fun getDateInString(OurDate: Date): String {
        ...
        stringDate = try {
            val formatter = SimpleDateFormat("vyyy-MM-dd'T'HH:mm:ss" , Locale("en"))
    ...
    fun SendDateToDate(OurDate: String): String {
        return try {
            val formatter = SimpleDateFormat("yyyy-MM-dd HH:mm a" , Locale("en"))
            val value = formatter.parse(OurDate)
            val dateFormatter = SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss" , Locale("en")) //this format changeable
    ...
    fun convertLongToTime(time: Long): String {
        val date = Date(time)
        val format = SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss" , Locale("en"))
    ...
    fun convertSingleDateNotification(startDate: String): String {
        ...
        val df = SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss" , Locale("en"))
        val currentDate = df.format(Calendar.getInstance().time)
        val subSplitedStartDate = startDate.split("-".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
        finalDate = when {
            isToday(startDate+"T00:00:00") -> "Today"
            isYesterday(startDate+"T00:00:00") -> "Yesterday"
            isTomorrow(startDate+"T00:00:00") -> "Tomorrow "
            getDaysDifference(startDate+"T00:00:00", currentDate) in 0..7 -> getDaysDifference(
                startDate+"T00:00:00",
                currentDate
            ).toString() +" days ago"
            else -> getDayName(startDate+"T00:00:00")  + ", " + getMonthNameByValue(
                Integer.parseInt(
    ...
    fun convertDateWithoutTime(startDate: String?): String {
        ...
        return if (startDate != null) {
            val df = SimpleDateFormat("yyyy-MM-dd HH:mm:ss" , Locale("en"))
            val splitedStartDate = startDate.split(" ".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
            val subSplitedStartDate = splitedStartDate[0].split("-".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
            getMonthFullNameByValue(Integer.parseInt(subSplitedStartDate[1])) + " " + subSplitedStartDate[2] + ", " + subSplitedStartDate[0]
        } else {
            " "
    ...
    fun convertSingleTime(startDate: String?): String {
        return if (startDate != null) {
            val splitedStartDate = if (startDate.contains("T")) startDate.split("T".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray() else startDate.split(" ".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
            val time1 = convertIn12HourFormatWithMinute(startDate)
            time1
        }
        else {
   ...
   /* fun convertSingleDateDoubleLine(startDate: String?): String {
        if (startDate != null) {
            ...
            finalDate = when {
                ...
                isYesterday(getLocalDate(startDate)) -> "${ActivityBase.activity.resources.getString(R.string.yesterday)} $time1"
    ...
    fun convertNotificationSingleDate(dataDate: String?): String? {
        ...
        try {
            val dateFormat = SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss" , Locale("en"))
    ...
    private fun convertIn12HourFormat(startTime: String): String {
        ...
        val sdfs = SimpleDateFormat("hh:mm a" , Locale("en"))
        val dt: Date
        try {
            dt = sdf.parse(time)
    ...
    private fun convertIn12HourFormatWithMinute(startTime: String): String {
        var startTime = startTime
        val splitedStartDate = if (startTime.contains("T")) startTime.split("T".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray() else startTime.split(" ".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
        val date = splitedStartDate[0]
        val time = splitedStartDate[1]

        val sdf = SimpleDateFormat("hh:mm:ss"  , Locale("en"))
        val sdfs = SimpleDateFormat("h:mm a"  , Locale("en"))
        val dt: Date
        try {
            dt = sdf.parse(time)
            startTime = sdfs.format(dt)
...
/*    fun getMonthDifference(startDate: String): Int {
    ...
    private fun isTodayDate(date: String): Boolean {
        val d = stringToDateJust(date)
        return android.text.format.DateUtils.isToday(d!!.time)
    }

    private fun isYesterdayDate(date: String): Boolean {
        val d = stringToDateJust(date)
        return android.text.format.DateUtils.isToday(d!!.time + android.text.format.DateUtils.DAY_IN_MILLIS)
    }

    fun isTomorrowDate(date: String): Boolean {
        val d = stringToDateJust(date)
        return android.text.format.DateUtils.isToday(d!!.time - android.text.format.DateUtils.DAY_IN_MILLIS)
    }
    ...
    fun stringToDateJust(dtString: String): Date? {
    ...
    fun getDayName(date: String): String? {
        ...
        val outFormat = SimpleDateFormat("EEE" , Locale("en"))
        name = outFormat.format(stringToDate(date))
        return name
    }

    private fun getDayNameDate(date: String): String? {
        var name: String? = null
        val outFormat = SimpleDateFormat("EEE" , Locale("en"))
        name = outFormat.format(stringToDateJust(date))
        return name
    }
    ...
    fun getDateForPastFromDays(days: Int): String {
        ...
        var dateFormat = SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss" , Locale("en"))
        return dateFormat.format(dateToUTC(calendar.time))
    }

    fun getDateForPastFromDays(days: Int, ourDate: String): String {
        val formatter = SimpleDateFormat("dd MMM yyyy" , Locale("en"))
        val value = formatter.parse(ourDate)
        var calendar: Calendar = Calendar.getInstance()
        calendar.time = value
        calendar.add(Calendar.DAY_OF_YEAR, days)
    ...
    fun getLocalDate(OurDate: String): String {
        return try {
            val simpleDateFormat = SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss" , Locale("en"))
    ...
    fun convertDate(startDate: String?): String {
        var startDate = startDate!!
        return if (startDate != null) {
            var finalDate: String? = null
            val splitedStartDate = if (startDate.contains("T")) startDate.split("T".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray() else startDate.split(" ".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
            val subSplitedStartDate = splitedStartDate[0].split("-".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
            finalDate = subSplitedStartDate[2]
            finalDate
        } else {
            " "
        }
    ...
    fun convertMonth(startDate: String?): String {
        var startDate = startDate!!
        return if (startDate != null) {
            var finalDate: String? = null
            val splitedStartDate = if (startDate.contains("T")) startDate.split("T".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray() else startDate.split(" ".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
            val subSplitedStartDate = splitedStartDate[0].split("-".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
            finalDate = getMonthNameByValue(Integer.parseInt(subSplitedStartDate[1]))
            finalDate
        } else {
            " "
        }
...
/*
    fun convertSingleDateTransaction(startDate: String): String {
        val startDate = startDate
        var finalDate: String? = null
        val df = SimpleDateFormat("yyyy-MM-dd" , Locale("en"))
        val subSplitedStartDate = startDate.split("-".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
        finalDate = when {
            isTodayDate(startDate) -> ActivityBase.activity.resources.getString(R.string.today)
            isYesterdayDate(startDate) -> ActivityBase.activity.resources.getString(R.string.yesterday)
            isTomorrowDate(startDate) -> "Tomorrow"
            else -> getDayNameDate(startDate)  + ", " + getMonthNameByValue(
                Integer.parseInt(
                    subSplitedStartDate[1]
                )
            ) + " " + subSplitedStartDate[2] + ", " + subSplitedStartDate[0]
    ...
    fun convertMonthYear(startDate: String?): String {
        var startDate = startDate!!
        return if (startDate != null) {
            var finalDate: String? = null
            val splitedStartDate = if (startDate.contains("T")) startDate.split("T".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray() else startDate.split(" ".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
            val subSplitedStartDate = splitedStartDate[0].split("-".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
            finalDate = getMonthFullNameByValue(Integer.parseInt(subSplitedStartDate[1]))
            finalDate +" " +subSplitedStartDate[0]
        } else {
            " "
        }
    ...
    fun convertSingleDateWallet(startDate: String): String {
        val startDate=  startDate
        var finalDate: String? = null
        val splitedStartDate = if (startDate.contains("T")) startDate.split("T".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray() else startDate.split(" ".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
        val subSplitedStartDate = splitedStartDate[0].split("-".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
        finalDate = getDayNameDate(startDate)  + ", " + getMonthNameByValue(
            Integer.parseInt(
                subSplitedStartDate[1]
            )
        ) + " " + subSplitedStartDate[2] + ", " + subSplitedStartDate[0]
